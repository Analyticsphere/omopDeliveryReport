...
test_that("parse_delivery_metrics extracts Connect participant filtering metrics", {
  delivery_data <- data.frame(
    ...
    value_as_string = rep("", 11),
    value_as_number = c(93, 15, 98, 10, 92, 16, 79, 26, 3, 561, 2),
...
test_that("prepare_overview_data uses delivery patients not in Connect in overview card", {
  metrics <- list(
  ...
  expect_equal(result$delivery_not_in_connect_display, "2")
  expect_equal(result$delivery_not_in_connect_warning, " warning")
  expect_match(result$delivery_not_in_connect_icon, "warning-icon")
})
